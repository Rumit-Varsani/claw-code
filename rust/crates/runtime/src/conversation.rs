...
// Pre-turn estimated-token threshold: compact before sending if the session's
// estimated token count exceeds this value. Useful for providers (e.g. Ollama)
// that do not return accurate input_token counts in API responses.
const PRE_COMPACTION_ESTIMATED_TOKENS_ENV_VAR: &str = "CLAW_AUTO_COMPACT_ESTIMATED_TOKENS";
const DEFAULT_PRE_COMPACTION_ESTIMATED_TOKENS: usize = 50_000;
...
pub struct ConversationRuntime<C, T> {
    ...
    pre_compaction_estimated_tokens_threshold: usize,
    ...
    T: ToolExecutor,
{
    ...
    pub fn new_with_features(
        ...
        Self {
            ...
            pre_compaction_estimated_tokens_threshold: pre_compaction_threshold_from_env(),
    ...
    #[must_use]
    pub fn with_pre_compaction_estimated_tokens_threshold(mut self, threshold: usize) -> Self {
        self.pre_compaction_estimated_tokens_threshold = threshold;
        self
    }
    ...
    pub fn run_turn(
        ...
        // Pre-turn compaction: compact based on estimated session size before
        // sending the request. This handles providers like Ollama that don't
        // return accurate input_token counts, preventing context overflow.
        let pre_turn_compaction = self.maybe_pre_turn_compact();
        ...
        let post_compaction = self.maybe_auto_compact();

        // Report whichever compaction fired (pre-turn takes priority; post-turn
        // would only fire if input_tokens is tracked accurately by the provider).
        let auto_compaction = pre_turn_compaction.or(post_compaction);
    ...
    // Compacts before the turn if the estimated session token count exceeds the
    // configured threshold. This fires regardless of whether the provider returns
    // accurate input_token counts, so it protects local models (e.g. Ollama) with
    // hard context limits from receiving an oversized prompt.
    fn maybe_pre_turn_compact(&mut self) -> Option<AutoCompactionEvent> {
        if self.pre_compaction_estimated_tokens_threshold == 0 {
            return None;
        }

        let estimated = estimate_session_tokens(&self.session);
        if estimated < self.pre_compaction_estimated_tokens_threshold {
            return None;
        }
...
/// Reads the pre-turn estimated-token compaction threshold from the environment.
/// Set `CLAW_AUTO_COMPACT_ESTIMATED_TOKENS=0` to disable pre-turn compaction.
#[must_use]
pub fn pre_compaction_threshold_from_env() -> usize {
    std::env::var(PRE_COMPACTION_ESTIMATED_TOKENS_ENV_VAR)
        .ok()
        .as_deref()
        .and_then(|raw| raw.trim().parse::<usize>().ok())
        .unwrap_or(DEFAULT_PRE_COMPACTION_ESTIMATED_TOKENS)
}
