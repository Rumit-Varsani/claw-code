...
/// Covers Anthropic Claude, OpenAI GPT, and Ollama/local model defaults.
/// Set `CLAW_CONTEXT_WINDOW` to override for any model.
fn context_window_for_model(model: &str) -> u32 {
    if let Ok(raw) = std::env::var("CLAW_CONTEXT_WINDOW") {
    ...
    // Strip routing prefix: "openai/qwen3.6:latest" → "qwen3.6:latest"
    ...
    if m.contains("claude") {
        ...
        // Ollama default num_ctx (qwen3, mistral, llama, etc.)
        131_072
    }
}

/// Renders a single-line token usage bar shown after every REPL turn.
///
/// Example: `Context: ████░░░░░░░░░░░░░░░░  12,345 / 131,072 tokens (9%)`
fn format_token_counter(used: usize, window: u32) -> String {
    ...
    format!(
        "Context: [{bar}]  {:>7} / {:>7} tokens ({:.0}%)",
        format_token_count(used),
        format_token_count(window as usize),
        pct * 100.0,
    )
}
...
/// Animating spinner that runs on a background thread and updates every 100ms,
/// showing elapsed time. Call `stop()` to clear it before printing output.
struct LiveSpinner {
    tx: Sender<()>,
    ...
    fn spawn() -> Self {
        ...
        let thread = thread::spawn(move || {
            ...
            loop {
                ...
                frame += 1;
                // Overwrite current line in place
                let _ = write!(stdout, "\r\x1b[2K{label}");
                let _ = stdout.flush();
            }
            // Clear the spinner line so streamed text starts cleanly
 ...
 ╚═════╝╚══════╝╚═╝  ╚═╝ ╚══╝╚══╝\x1b[0m \x1b[38;5;208mCode\x1b[0m 🦞\n\n\
  ...
  Type \x1b[1m/help\x1b[0m for commands · \x1b[1m/status\x1b[0m for live context · \x1b[2m/resume latest\x1b[0m jumps back to the newest session · \x1b[1m/diff\x1b[0m then \x1b[1m/commit\x1b[0m to ship · \x1b[2mTab\x1b[0m for workflow completions · \x1b[2mShift+Enter\x1b[0m for newline",
    ...
    fn run_turn(&mut self, input: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Animated spinner that shows elapsed time until the first text token
        // arrives. Runs on a background thread so the main thread can block on
        // the model response without the UI freezing.
        let spinner_handle = LiveSpinner::spawn();
        ...
        match result {
            Ok(summary) => {
                ...
                let done_label = format!("✨ Done · {elapsed_s:.1}s");
                ...
                println!("{}", format_token_counter(estimated, window));
